...
object tweets {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val tweet = sqlContext.read.option("header","true").option("inferSchema","true").csv(inputFile)

    val inp_tweet = tweet.filter(tweet.col("text").isNotNull).drop("tweet_coord", "airline_sentiment_gold", "negativereason_gold")

    //split data into training and test
    val Array(training, test) = inp_tweet.randomSplit(Array(0.8, 0.2))

    //creating tokenizer pipeline
